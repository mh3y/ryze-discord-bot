...
async def sync_all_calendars_portal() -> dict:
    """
    Portal-first calendar sync — reads class groups from Supabase, not the local bot DB.

    Flow:
      1. GET /api/bot/classes → list of classes with google_calendar_id from Supabase
      2. For each class that has a google_calendar_id, fetch events from Google Calendar
      3. POST /api/bot/sync-lessons → push all events to Supabase

    This is the correct path for the CRM pipeline. The old sync_all_calendars()
    reads from the local bot PostgreSQL which is empty in production (the old OCI
    stack is deprecated — Supabase is the source of truth).

    Returns the same dict shape as sync_all_calendars() for compatibility with
    the CalendarSyncCog.
    """
    ...
    if not config.DASHBOARD_API_KEY or not config.PORTAL_API_URL:
        log.warning("[portal] sync_all_calendars_portal: DASHBOARD_API_KEY or PORTAL_API_URL not set — skipping.")
        return totals
    ...
    try:
        ...
        log.error("[portal] sync_all_calendars_portal: failed to fetch classes: %s", exc)
        totals["errors"] += 1
        return totals

    classes_with_cal = [c for c in portal_classes if c.get("google_calendar_id")]
    if not classes_with_cal:
        log.warning(
            "[portal] sync_all_calendars_portal: no portal classes have google_calendar_id set. "
            "Run /discover_classes to provision classes from Google Calendar first."
        )
        return totals

    log.info("[portal] sync_all_calendars_portal: syncing %d class(es) from Supabase.", len(classes_with_cal))

    payload: list[dict] = []

    for cls in classes_with_cal:
        cal_id    = cls["google_calendar_id"]
        class_id  = cls["id"]
        class_name = cls.get("name", str(class_id))
        try:
            events = fetch_upcoming_events(cal_id, config.CALENDAR_SYNC_DAYS_AHEAD)
            for event in events:
                if event.get("cancelled"):
                    totals["cancelled"] += 1
                    continue
                ...
                payload.append({
                    "google_event_id": event["google_event_id"],
                    "class_id":        class_id,
                    "title":           event.get("title") or class_name,
                    "description":     event.get("description"),
                    "scheduled_at":    start.isoformat() if isinstance(start, datetime) else str(start),
                    "duration_min":    duration_min,
                    "meet_link":       event.get("meet_link"),
                    "status":          "scheduled",
                })
            log.info("[portal] Fetched %d events for class %r (cal=%s).", len(events), class_name, cal_id)
        except Exception as exc:
            log.exception("[portal] Failed to fetch events for class %r (cal=%s): %s", class_name, cal_id, exc)
            totals["errors"] += 1

    if not payload:
        log.info("[portal] sync_all_calendars_portal: no events to push.")
        return totals

    try:
        PortalAPIClient2, PortalAPIError2 = _get_portal_client()
        async with PortalAPIClient2() as client:
            result = await client.sync_lessons(payload)
            totals["created"] = result.get("created", 0)
            totals["updated"] = result.get("updated", 0)
            log.info(
                "[portal] sync_all_calendars_portal: pushed %d lessons → created=%d updated=%d.",
                len(payload), totals["created"], totals["updated"],
            )
    except Exception as exc:
        log.error("[portal] sync_all_calendars_portal: lesson push failed: %s", exc)
        totals["errors"] += 1
